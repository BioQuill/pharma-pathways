...
import { Check, Crown, FileText, TrendingUp, Eye, BarChart3, Target, Shield, Percent, Calculator, DollarSign, PieChart, Clock, Database, RefreshCw, CheckCircle, Pill, Layers, Globe, Settings } from "lucide-react";
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-xs text-muted-foreground">Phase</p><p className="font-bold">Phase III</p></div>
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-xs text-muted-foreground">Indication</p><p className="font-bold text-sm">Obesity / T2D</p></div>
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-xs text-muted-foreground">Company</p><p className="font-bold text-sm text-[hsl(142,70%,30%)]">Eli Lilly</p></div>
          </div>
          <div className="grid grid-cols-3 gap-3 mt-3">
            <div className="text-center p-3 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200"><p className="text-xs text-muted-foreground">Composite Score</p><p className="text-2xl font-bold text-[hsl(142,76%,36%)]">74.2</p></div>
            <div className="text-center p-3 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200"><p className="text-xs text-muted-foreground">LPI-3 Score</p><p className="text-2xl font-bold text-blue-600">71.8%</p></div>
            <div className="text-center p-3 rounded-lg bg-muted/50"><p className="text-xs text-muted-foreground">TTM</p><p className="text-2xl font-bold">18 mo</p></div>
          </div>
        </CardContent>
      </Card>
      <div className="text-center pt-4 border-t">
        <p className="text-xs text-muted-foreground mb-2">This is a sample preview. Actual reports include deeper methodology, interactive charts, and actionable insights.</p>
        <Badge variant="outline" className="text-xs"><Percent className="h-3 w-3 mr-1" />10 Sections · 10,000+ Monte Carlo Simulations</Badge>
      </div>
    </div>
  </div>
);
    ...
    <Card className="relative flex flex-col border-2 border-primary shadow-lg shadow-primary/10">
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center gap-2 mb-1">
          <div className="p-2 rounded-lg bg-primary text-primary-foreground"><Settings className="h-6 w-6" /></div>
          <div><CardTitle className="text-base">1 Therapeutic Area</CardTitle><p className="text-xs font-medium text-muted-foreground">Your TA. Your Phases. Your Price.</p></div>
        </div>
      </CardHeader>
      <CardContent className="flex-1 pb-2 space-y-4">
        <div className="space-y-2">
          ...
          <Select value={selectedTA} onValueChange={setSelectedTA}>
            <SelectTrigger className="w-full text-xs"><SelectValue placeholder="Select a therapeutic area" /></SelectTrigger>
            <SelectContent>{TA_ORDER.map(ta => (<SelectItem key={ta} value={ta} className="text-xs">{ta} — {TA_MATRIX[ta].total.toLocaleString()} trials</SelectItem>))}</SelectContent>
          </Select>
        </div>
        <div className="space-y-2">
          <Label className="text-xs font-semibold">Select Phases</Label>
          <div className="flex gap-1">
            {[{ key: "all", label: "All" },{ key: "1", label: "Phase 1" },{ key: "2", label: "Phase 2" },{ key: "3", label: "Phase 3" }].map(opt => (
              <button key={opt.key} onClick={() => handlePhaseToggle(opt.key)} className={`flex-1 py-1.5 px-2 text-xs font-medium rounded-md border transition-colors ${phases.includes(opt.key) ? "bg-primary text-primary-foreground border-primary" : "bg-background text-foreground border-input hover:bg-accent"}`}>{opt.label}</button>
            ))}
          </div>
        </div>
        {result ? (
          ...
          <div className="text-center py-3 rounded-lg bg-muted/50 border"><p className="text-sm text-muted-foreground">Select a TA to see pricing</p></div>
        )}
        <ul className="space-y-1.5">
          {["Full DD reports for every trial profile in selected scope","TA Composite Index analytics","Head-to-head molecule comparison tools","Pipeline trend & white-space gap analysis","M&A and in-licensing target identification","Competitive intelligence sweep across selected scope","Phase progression tracking","1-year live monitoring & automated alerts"].map((f, i) => (
            <li key={i} className="flex items-start gap-1.5 text-xs"><Check className="h-3 w-3 text-[hsl(142,76%,36%)] mt-0.5 flex-shrink-0" /><span>{f}</span></li>
          ))}
        </ul>
      </CardContent>
      <CardFooter className="pt-2">
        <Button className="w-full" disabled={!selectedTA} size="sm">{selectedTA ? "Configure & Add to Cart" : "Select a TA to continue"}</Button>
      </CardFooter>
    </Card>
  );
};
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="text-center mb-12">
        ...
        <h1 className="text-4xl font-bold mb-4">Pharmaceutical Intelligence for <span className="text-primary">Smarter Investments</span></h1>
        <p className="text-xl text-muted-foreground max-w-3xl mx-auto">From single molecule reports to full enterprise access across all 20 therapeutic areas. Every plan includes 1-year monitoring and compounded due diligence reports.</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mb-16">
        ...
        <Card className="relative flex flex-col border transition-colors duration-150 ease-in-out hover:bg-[#FFFBEB] hover:border-[#F59E0B]">
          <CardHeader className="pb-2">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-2 rounded-lg bg-muted"><Pill className="h-6 w-6" /></div>
              <div><CardTitle className="text-base">1 Molecule</CardTitle><p className="text-xs font-medium text-muted-foreground">Single Molecule — All Phases</p></div>
            </div>
            <CardDescription className="text-xs">1 molecule of your choice — all development phases included</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 pb-2">
            <div className="mb-3"><span className="text-2xl font-bold">$5,000</span><span className="text-muted-foreground text-xs ml-1">/ year</span></div>
            <ul className="space-y-1.5">
              {["Full compounded due diligence report (print & export)","Molecule Score Card & Composite Score","LPI-3 launch probability analysis + extended","Investment Score Assessment","Pricing & Access Index (Model 1 + Model 2 + Triangulation)","Monte Carlo peak sales simulation","Market & Regulatory analysis (10 markets)","PTRS Technical & Regulatory Success","CAPM Alpha Signals","TTM Time-to-Market estimate","1-year trial status monitoring & alerts"].map((f, i) => (
                <li key={i} className="flex items-start gap-1.5 text-xs"><Check className="h-3 w-3 text-[hsl(142,76%,36%)] mt-0.5 flex-shrink-0" /><span>{f}</span></li>
              ))}
            </ul>
          </CardContent>
          <CardFooter className="flex flex-col gap-2 pt-2">
            <Button variant="outline" size="sm" className="w-full">Purchase Report</Button>
            <Dialog>
              <DialogTrigger asChild><Button variant="ghost" size="sm" className="w-full text-xs"><Eye className="h-3 w-3 mr-1" />Preview Sample Report (Orforglipron)</Button></DialogTrigger>
              <DialogContent className="max-w-2xl">
                <DialogHeader><DialogTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary" />Sample Compounded Report — Orforglipron (Eli Lilly)</DialogTitle></DialogHeader>
        ...
        {/* Full Access */}
        <Card className="relative flex flex-col border transition-colors duration-150 ease-in-out hover:bg-[#FFFBEB] hover:border-[#F59E0B]">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2"><Badge className="bg-[hsl(142,76%,36%)] text-white">Best Value</Badge></div>
          <CardHeader className="pb-2">
            <div className="flex items-center gap-2 mb-1">
              <div className="p-2 rounded-lg bg-muted"><Crown className="h-6 w-6" /></div>
              <div><CardTitle className="text-base">Full Access</CardTitle><p className="text-xs font-medium text-muted-foreground">All 20 TAs — All Phases — Enterprise</p></div>
            </div>
            <CardDescription className="text-xs">21,739 trial profiles — all 20 TAs — all phases</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 pb-2">
            <div className="mb-1"><span className="text-2xl font-bold">$300,000</span><span className="text-muted-foreground text-xs ml-1">/ year (starting)</span></div>
            ...
            <ul className="space-y-1.5">
              {["All 20 therapeutic areas — 21,739 trial profiles","Unlimited DD reports & comparisons","Full molecule comparison across all TAs","Enterprise portfolio dashboard","Custom scoring & model configurations","White-label reporting","SSO & team management","Dedicated account manager","Quarterly model calibration consultations","Priority support & SLA"].map((f, i) => (
                <li key={i} className="flex items-start gap-1.5 text-xs"><Check className="h-3 w-3 text-[hsl(142,76%,36%)] mt-0.5 flex-shrink-0" /><span>{f}</span></li>
              ))}
            </ul>
          </CardContent>
          <CardFooter className="pt-2">
            <Button className="w-full bg-[hsl(142,70%,45%)] hover:bg-[hsl(142,70%,40%)] text-white" size="sm">Contact Sales</Button>
          </CardFooter>
        </Card>
      </div>
      ...
      <div className="mb-16">
        ...
        <p className="text-center text-muted-foreground mb-8 max-w-2xl mx-auto">Every tier includes a compounded Full Due Diligence Report combining all analytical modules into one deliverable.</p>
          ...
          {[
            ...
            <Card key={item.title} className="text-center">
              <CardContent className="pt-6 pb-4">
                <div className="mx-auto w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary mb-3">{item.icon}</div>
      ...
      <Card className="bg-gradient-to-r from-primary/5 to-primary/10 border-primary/20 mb-16">
        <CardContent className="p-8">
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <div><div className="text-4xl font-bold text-primary mb-2">$300K</div><p className="text-sm font-medium">Full pipeline access</p><p className="text-xs text-muted-foreground">21,739 trial profiles</p></div>
            <div><div className="text-4xl font-bold text-primary mb-2">82%</div><p className="text-sm font-medium">LPI-3 model accuracy (AUC-ROC)</p></div>
            <div><div className="text-4xl font-bold text-primary mb-2">21,739+</div><p className="text-sm font-medium">Trial profiles across 20 TAs</p></div>
          </div>
        </CardContent>
      </Card>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="text-center p-4 font-semibold text-[hsl(142,76%,36%)]"><div className="flex items-center justify-center gap-2"><CheckCircle className="h-4 w-4" />BioQuill</div></th>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={idx} className="border-b hover:bg-muted/30">
                      <td className="p-4 flex items-center gap-2"><span className="text-muted-foreground">{row.icon}</span>{row.metric}</td>
                      ...
                      <td className="text-center p-4"><Badge variant="outline" className="bg-green-50 text-[hsl(142,76%,36%)] border-green-200">{row.adv}</Badge></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-4">
          {[
            ...
            { q: "What molecule comparison features are available?", a: "TA packages include molecule comparison tools: head-to-head score comparison, LPI-3 side-by-side, competitive positioning analysis, TTM benchmarking, and risk-adjusted metrics comparison." },
            ...
            <Card key={idx}><CardHeader><CardTitle className="text-lg">{faq.q}</CardTitle></CardHeader><CardContent><p className="text-muted-foreground">{faq.a}</p></CardContent></Card>
          ))}
        </div>
      </div>
      ...
      <div className="text-center mt-16">
        ...
        <p className="text-muted-foreground mb-6">Start with a single molecule report, configure your therapeutic area package, or contact us for full enterprise access.</p>
        <div className="flex justify-center gap-4">
          <Button size="lg">Explore Platform</Button>
          <Button size="lg" className="bg-[hsl(142,70%,45%)] hover:bg-[hsl(142,70%,40%)] text-white">Contact Sales</Button>
        </div>
      </div>
    </div>
  );
