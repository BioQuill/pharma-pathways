...
export function CAPMAlphaSignals({ molecules }: { molecules: MoleculeProfile[] }) {
      ...
      <div className="space-y-6">
        {!simulatorMolecule && (
          <Card>
            ...
            <CardContent>
              <MoleculePicker molecules={molecules} value={localMol} onChange={setLocalMol} label="Molecule for CAPM Analysis" />
            </CardContent>
          </Card>
        )}
  ...
  // Context chart: scatter plot / alpha visualization
  const contextChart = capmResult ? (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold">Alpha Signals Overview</h4>
      <div className="grid gap-4 md:grid-cols-3">
        <div className="p-3 rounded-lg border-l-4" style={{ borderLeftColor: getAlphaColor(capmResult.alpha1) }}>
          <p className="text-xs text-muted-foreground">α₁ — Historical</p>
          <p className="text-xl font-bold" style={{ color: getAlphaColor(capmResult.alpha1) }}>{fmtSignedPct(capmResult.alpha1)}</p>
          <Badge variant="outline" className="text-xs mt-1">{getAlphaLabel(capmResult.alpha1)}</Badge>
        </div>
        <div className="p-3 rounded-lg border-l-4" style={{ borderLeftColor: getAlphaColor(capmResult.alpha2) }}>
          <p className="text-xs text-muted-foreground">α₂ — Pipeline</p>
          <p className="text-xl font-bold" style={{ color: getAlphaColor(capmResult.alpha2) }}>{fmtSignedPct(capmResult.alpha2)}</p>
          <Badge variant="outline" className="text-xs mt-1">{getAlphaLabel(capmResult.alpha2)}</Badge>
        </div>
        <div className="p-3 rounded-lg border-l-4" style={{ borderLeftColor: getAlphaColor(capmResult.deltaAlpha) }}>
          <p className="text-xs text-muted-foreground">Δα — Divergence</p>
          <p className="text-xl font-bold" style={{ color: getAlphaColor(capmResult.deltaAlpha) }}>{fmtSignedPct(capmResult.deltaAlpha)}</p>
  ...
  // Parameters: beta inputs + signal summary table
  const parametersContent = (
    <div className="space-y-6">
      {/* Molecule Picker (inline) */}
      {!simulatorMolecule && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2"><Target className="h-4 w-4 text-primary" />Select Molecule</CardTitle>
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-1.5">
              ...
              <Select value={mechanism} onValueChange={v => { setMechanism(v as MechanismType); setUserAdjusted(true); }}>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={designation} onValueChange={v => { setDesignation(v as DesignationType); setUserAdjusted(true); }}>
                <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={sponsorTier} onValueChange={v => { setSponsorTier(v as SponsorTier); setUserAdjusted(true); }}>
                <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={priorData} onValueChange={v => { setPriorData(v as PriorData); setUserAdjusted(true); }}>
                <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                <SelectContent>
  ...
  return (
    <SimulatorLayout
      ...
      badgeLabel="Alpha (α₁)"
      badgeSuffix="%"
      principle={`${selectedMol.name} shows a historical alpha of ${alphaValue}% vs CAPM expected return, indicating ${capmResult && capmResult.alpha1 > 0 ? 'outperformance' : 'underperformance'} relative to TA risk.`}
